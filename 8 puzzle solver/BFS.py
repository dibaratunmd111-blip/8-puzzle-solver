    ...
    def neighbors(self):
        ...
        for dr, dc in moves:
            ...
            if 0 <= nr < 3 and 0 <= nc < 3:
                ...
                res.append(PuzzleState(tuple(b), self))
        return res

def solve_bfs(start, goal):
    queue = deque([PuzzleState(start)])
    visited = {start}
    visited_states = []
    while queue:
        curr = queue.popleft()
...
def main(page: ft.Page):
    page.title = "8 Puzzle BFS Solver"
    ...
    def solve(e):
        ...
        start = tuple(board)
        t0 = time.time()
        goal_node, visited_states = solve_bfs(start, goal)
        t1 = time.time()
        ...
        if goal_node:
            ...
            for state in path:
                for i in range(9): board[i] = state[i]
                draw()
                time.sleep(0.15)
    ...
    page.add(
        ft.Text("8-Puzzle BFS Solver", size=32, weight="bold"),
        grid,
        ft.Row(
            [
                ft.ElevatedButton("Shuffle", on_click=randomize),
        ...
        # Side-by-side containers for logs
        ft.Row(
            [
                ft.Container(
                    content=ft.Column([
